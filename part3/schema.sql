...
drop function consulta_check;
drop function analise_check;

create function consulta_check (n_doente int, d date, i varchar)
  returns int
as
$$
  declare counter int;
  begin
    ...
    from consulta
    where num_doente = n_doente and data = d and nome_instituicao = i;
    return counter;
  end
$$
language plpgsql;

create function analise_check (n_cedula int, e char)
  returns int
as
$$
  declare counter int;
  begin
    select count(*) into counter
    from medico
    where num_cedula = n_cedula and especialidade = e;
    return counter;
  end
$$
language plpgsql;
...
CREATE TABLE consulta (
    ...
    CHECK(consulta_check(num_doente, data, nome_instituicao) = 0)
);
...
CREATE TABLE prescricao (
    ...
    FOREIGN KEY(num_cedula, num_doente, data) REFERENCES consulta(num_cedula, num_doente, data)
);
...
CREATE TABLE analise (
    ...
    CHECK(analise_check(num_cedula, especialidade) > 0)
);
...
CREATE TABLE prescricao_venda (
    ...
    FOREIGN KEY(num_cedula, num_doente, data, substancia) REFERENCES prescricao(num_cedula, num_doente, data, substancia),
